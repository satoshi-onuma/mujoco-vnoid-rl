...
print("=" * 70)
print("🚀 Vnoid Humanoid 分散学習スクリプト")
print("=" * 70)
...
print("✅ Ray初期化完了")
print(f"  ノード数: {len(ray.nodes())}")
print(f"  利用可能CPU: {ray.cluster_resources().get('CPU', 0)}")
print(f"  利用可能GPU: {ray.cluster_resources().get('GPU', 0)}")
print(f"  利用可能メモリ: {ray.cluster_resources().get('memory', 0) / (1024**3):.1f} GB")
print(f"  ダッシュボード: http://localhost:8265")
print()
