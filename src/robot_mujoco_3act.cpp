...
void RobotMujoco::Actuate(Timer& timer, Base& base, vector<Joint>& joint){
    ...
    for (int i = 0; i < joint.size(); ++i) {
        ...
        // MuJoCo: per-joint position + velocity + motor torque sum to u_ref + P + D (see MJCF actuators).
        const int b = 3 * i;
        d->ctrl[b + 0] = joint[i].q_ref;
        d->ctrl[b + 1] = joint[i].dq_ref;
        d->ctrl[b + 2] = joint[i].u_ref;
	}
}
